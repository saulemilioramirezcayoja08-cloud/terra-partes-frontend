...
export class CustomerCreate implements OnInit {
  ...
  // ultimo cliente creado
  protected last_customer = signal<customer_create_response | null>(null);
  ...
  // computados
  protected has_last_customer = computed(() => this.last_customer() !== null);
  ...
  private load_form_from_service(): void {
    const data = this.context_form_service.get_customer_data(this.context);
    ...
    this.last_customer.set(data.last_customer);
  }

  // guarda datos del formulario al servicio
  private save_form_to_service(): void {
    this.context_form_service.update_customer_form(this.context, {
  ...
  protected on_cancel(): void {
    this.context_form_service.clear_customer(this.context);
    this.load_form_from_service();
  }
  ...
  protected on_save(): void {
    ...
    this.customer_service.create(request).subscribe({
      next: (response) => {
        this.context_form_service.set_last_customer(this.context, response);
        this.last_customer.set(response);
        this.loading.set(false);

        // limpia formulario pero mantiene last_customer
        this.context_form_service.update_customer_form(this.context, {
          user_id: null,
          tax_id: '',
          name: '',
  ...
  protected on_preview(): void {
    this.router.navigate(['/layout/customers/preview']);
  }
}
