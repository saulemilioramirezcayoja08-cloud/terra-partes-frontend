...
@Component({
  selector: 'app-income-concept-create',
  imports: [Breadcrumb, FormsModule],
  templateUrl: './income-concept-create.html',
  styleUrl: './income-concept-create.css',
})
export class IncomeConceptCreate implements OnInit {
  // dependencias
  ...
  protected last_concept = signal<income_concept_create_response | null>(null);
  ...
  private load_form_from_service(): void {
    const data = this.context_form_service.get_income_concept_data(this.context);
    this.selected_user.set(data.form.user_id);
    this.name.set(data.form.name);
    this.description.set(data.form.description);
    this.last_concept.set(data.last_income_concept);
  }

  // guarda datos del formulario al servicio
  private save_form_to_service(): void {
    this.context_form_service.update_income_concept_form(this.context, {
  ...
  protected on_cancel(): void {
    this.context_form_service.clear_income_concept(this.context);
    this.load_form_from_service();
  }
  ...
  protected on_save(): void {
    ...
    this.income_service.create_concept(request).subscribe({
      next: (response) => {
        this.context_form_service.set_last_income_concept(this.context, response);
  ...
  protected on_preview(): void {
    this.router.navigate(['/layout/incomes/concepts/preview']);
  }
}
