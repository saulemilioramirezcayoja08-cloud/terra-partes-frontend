...
@Component({
  ...
  imports: [Breadcrumb],
  templateUrl: './expense-preview.html',
  styleUrl: './expense-preview.css',
})
export class ExpensePreview implements OnInit {
  // dependencias
  ...
  // datos del egreso
  protected expense = signal<expense_detail_response | null>(null);
  ...
  protected total_literal = computed(() => {
    const total = this.expense()?.totals.total ?? 0;
  ...
  // carga egreso desde api o desde servicio
  private load_expense(): void {
    ...
    if (id) {
      ...
      // desde create: usar last_expense
      this.load_from_service();
    }
  }

  // carga egreso desde api
  private load_from_api(id: number): void {
    this.loading.set(true);
    this.error_message.set(null);

    this.expense_service.get_expense(id).subscribe({
      next: (response) => {
        this.expense.set(response);
        this.loading.set(false);
      },
      error: (err) => {
        this.error_message.set(err.error?.message ?? 'Error al cargar el egreso');
        this.loading.set(false);
      }
    });
  }

  // carga egreso desde servicio
  private load_from_service(): void {
    const last_expense = this.context_form_service.get_last_expense(this.context);
    if (last_expense) {
      this.expense.set(last_expense);
    } else {
      this.router.navigate(['/layout/expenses/create']);
    }
  }
  ...
  protected go_back(): void {
    if (window.history.length > 1) {
      ...
      this.router.navigate(['/layout/expenses/list']);
    }
  }
