...
export class SaleList implements OnInit {
  ...
  // datos de ventas
  protected data = signal<sale_list_response[]>([]);
  ...
  private load_statuses(): void {
    ...
    const sale_statuses = statuses.filter(s =>
      ['PENDING', 'COMPLETED', 'VOIDED'].includes(s.code)
    );
    this.statuses.set(sale_statuses);
  }
  ...
  // carga ventas
  private load_sales(): void {
    this.loading.set(true);

    this.sale_service.get_sales(
      this.current_page(),
      this.page_size(),
      this.selected_user(),
