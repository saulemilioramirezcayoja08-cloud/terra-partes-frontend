...
@Component({
  ...
  imports: [Breadcrumb, Paginator, FormsModule, DatePipe],
  templateUrl: './inventory-valuation.html',
  styleUrl: './inventory-valuation.css',
})
export class InventoryValuation implements OnInit {
  // dependencias
  private inventory_service = inject(InventoryService);
  private category_service = inject(CategoryService);
  private platform_id = inject(PLATFORM_ID);

  // datos de valorizacion
  protected data = signal<inventory_valuation_response[]>([]);
  ...
  protected expanded_items = signal<Set<string>>(new Set());
  ...
  protected breadcrumb_items = signal<breadcrumb_item[]>([
    { label: 'Inventario' },
    { label: 'Valorización', active: true }
  ]);
  ...
  // genera key unica para producto+almacen
  protected get_item_key(item: inventory_valuation_response): string {
    return `${item.product.id}-${item.warehouse.id}`;
  }

  // toggle expandir/colapsar lotes
  protected toggle_expand(item: inventory_valuation_response): void {
    const key = this.get_item_key(item);
  ...
  protected is_expanded(item: inventory_valuation_response): boolean {
    return this.expanded_items().has(this.get_item_key(item));
  }

  // calcula total de original de los lotes
  protected get_total_original(item: inventory_valuation_response): number {
    return item.lots.reduce((sum, lot) => sum + lot.original, 0);
  }

  // calcula total de remaining de los lotes
  protected get_total_remaining(item: inventory_valuation_response): number {
    return item.lots.reduce((sum, lot) => sum + lot.remaining, 0);
  }

  // calcula valor total de los lotes
  protected get_total_value(item: inventory_valuation_response): number {
    return item.lots.reduce((sum, lot) => sum + lot.value, 0);
  }

  // calcula costo promedio ponderado: total_value / total_remaining
  protected get_average_cost(item: inventory_valuation_response): number {
    const total_remaining = this.get_total_remaining(item);
    if (total_remaining === 0) return 0;
    return this.get_total_value(item) / total_remaining;
  }
