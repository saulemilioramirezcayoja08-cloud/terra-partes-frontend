...
@Component({
  selector: 'app-quotation-select-product',
  imports: [CommonModule, FormsModule, DecimalPipe],
  templateUrl: './quotation-select-product.html',
  styleUrl: './quotation-select-product.css'
})
export class QuotationSelectProduct implements OnInit, OnDestroy {
  ...
  readonly inCartIds = this.quotationCartService.productIds;
...
private activeFilters: { name?: string; code?: string; sku?: string } = {};
  ...
  ngOnInit(): void {
    this.loadProducts();
    if (this.isBrowser) this.setupKeyboardShortcuts();
  }
  ...
  addProductToQuotation(product: ProductListResponse): void {
    if (this.isInCart(product.id)) return;
    this.quotationCartService.addDetail({
  ...
  private setupKeyboardShortcuts(): void {
    ...
    this.keyboardListener = (e: KeyboardEvent) => {
      if (e.key === 'Escape') this.router.navigate(['/quotation/create']);
    };
