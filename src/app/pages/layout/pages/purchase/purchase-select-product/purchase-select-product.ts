...
@Component({
  selector: 'app-purchase-select-product',
  imports: [CommonModule, FormsModule, DecimalPipe],
  templateUrl: './purchase-select-product.html',
  styleUrl: './purchase-select-product.css'
})
export class PurchaseSelectProduct implements OnInit, OnDestroy {
  ...
  readonly inCartIds = this.purchaseCartService.productIds;
  ...
  addProductToPurchase(product: ProductListResponse): void {
    if (this.isInCart(product.id)) return;
    this.purchaseCartService.addDetail({
  ...
  private setupKeyboardShortcuts(): void {
    ...
    this.keyboardListener = (e: KeyboardEvent) => {
      if (e.key === 'Escape') this.router.navigate(['/purchase/create']);
    };
